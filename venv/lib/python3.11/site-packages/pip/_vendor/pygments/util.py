...
def shebang_matches(text, pattern):
    ...
    if first_line.startswith('#!'):
        try:
            ...
            return False
        pattern = re.compile(rf'^{pattern}(\.(exe|cmd|bat|bin))?$', re.IGNORECASE)
        if pattern.search(found) is not None:
            return True
    return False


def doctype_matches(text, pattern):
    ...
    if m is None:
        return False
    doctype = m.group(1)
    return re.compile(pattern, re.I).match(doctype.strip()) is not None
...
class Future:
    ...
    Handled specially in patternLexerMeta, to support pattern string construction at
    first use.
    """
    def get(self):
