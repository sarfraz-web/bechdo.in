...
def _set_requirement_extras(req: Requirement, new_extras: Set[str]) -> Requirement:
    ...
    # ireq.req is a valid requirement so the pattern should always match
    assert (
        match is not None
    ), f"pattern match on requirement {req} failed, this should never happen"
    pre: Optional[str] = match.group(1)
    post: Optional[str] = match.group(3)
    assert (
        pre is not None and post is not None
    ), f"pattern group selection for requirement {req} failed, this should never happen"
