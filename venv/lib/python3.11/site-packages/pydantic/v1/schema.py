...
def field_type_schema(
    ...
    *,
    ...
    if field.shape in {
        ...
        pattern = getattr(key_field.type_, 'pattern', None)
        items_schema, f_definitions, f_nested_models = field_singleton_schema(
            field,
            by_alias=by_alias,
            model_name_map=model_name_map,
        ...
        nested_models.update(f_nested_models)
        if pattern:
            # Dict keys have a pattern pattern
            # items_schema might be a schema or empty dict, add it either way
            f_schema['patternProperties'] = {ConstrainedStr._get_pattern(pattern): items_schema}
        if items_schema:
...
field_class_to_schema: Tuple[Tuple[Any, Dict[str, Any]], ...] = (
    ...
    (Pattern, {'type': 'string', 'format': 'pattern'}),
    (str, {'type': 'string'}),
    ...
    def go(type_: Any) -> Type[Any]:
        ...
        if isinstance(type_, type):
            if issubclass(type_, (SecretStr, SecretBytes)):
                ...
                attrs = ('max_length', 'min_length', 'pattern')
