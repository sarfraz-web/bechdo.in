...
_url_pattern_cache = None
_multi_host_url_pattern_cache = None
_ascii_domain_pattern_cache = None
_int_domain_pattern_cache = None
_host_pattern_cache = None
...
_scheme_pattern = r'(?:(?P<scheme>[a-z][a-z0-9+\-.]+)://)?'  # scheme https://tools.ietf.org/html/rfc3986#appendix-A
_user_info_pattern = r'(?:(?P<user>[^\s:/]*)(?::(?P<password>[^\s/]*))?@)?'
_path_pattern = r'(?P<path>/[^\s?#]*)?'
_query_pattern = r'(?:\?(?P<query>[^\s#]*))?'
_fragment_pattern = r'(?:#(?P<fragment>[^\s#]*))?'


def url_pattern() -> Pattern[str]:
    global _url_pattern_cache
    if _url_pattern_cache is None:
        _url_pattern_cache = re.compile(
            rf'{_scheme_pattern}{_user_info_pattern}{_host_pattern}{_path_pattern}{_query_pattern}{_fragment_pattern}',
            re.IGNORECASE,
        )
    return _url_pattern_cache


def multi_host_url_pattern() -> Pattern[str]:
    """
    Compiled multi host url pattern.

    Additionally to `url_pattern` it allows to match multiple hosts.
    E.g. host1.db.net,host2.db.net
    """
    global _multi_host_url_pattern_cache
    if _multi_host_url_pattern_cache is None:
        _multi_host_url_pattern_cache = re.compile(
            rf'{_scheme_pattern}{_user_info_pattern}'
            r'(?P<hosts>([^/]*))'  # validation occurs later
            rf'{_path_pattern}{_query_pattern}{_fragment_pattern}',
            re.IGNORECASE,
        )
    return _multi_host_url_pattern_cache


def ascii_domain_pattern() -> Pattern[str]:
    global _ascii_domain_pattern_cache
    if _ascii_domain_pattern_cache is None:
        ascii_chunk = r'[_0-9a-z](?:[-_0-9a-z]{0,61}[_0-9a-z])?'
        ascii_domain_ending = r'(?P<tld>\.[a-z]{2,63})?\.?'
        _ascii_domain_pattern_cache = re.compile(
            fr'(?:{ascii_chunk}\.)*?{ascii_chunk}{ascii_domain_ending}', re.IGNORECASE
        )
    return _ascii_domain_pattern_cache


def int_domain_pattern() -> Pattern[str]:
    global _int_domain_pattern_cache
    if _int_domain_pattern_cache is None:
        int_chunk = r'[_0-9a-\U00040000](?:[-_0-9a-\U00040000]{0,61}[_0-9a-\U00040000])?'
        int_domain_ending = r'(?P<tld>(\.[^\W\d_]{2,63})|(\.(?:xn--)[_0-9a-z-]{2,63}))?\.?'
        _int_domain_pattern_cache = re.compile(fr'(?:{int_chunk}\.)*?{int_chunk}{int_domain_ending}', re.IGNORECASE)
    return _int_domain_pattern_cache


def host_pattern() -> Pattern[str]:
    global _host_pattern_cache
    if _host_pattern_cache is None:
        _host_pattern_cache = re.compile(
            _host_pattern,
            re.IGNORECASE,
        )
    return _host_pattern_cache
...
class AnyUrl(str):
    ...
    def validate(cls, value: Any, field: 'ModelField', config: 'BaseConfig') -> 'AnyUrl':
        ...
        # the pattern should always match, if it doesn't please report with details of the URL tried
        assert m, 'URL pattern failed unexpectedly'
    ...
    def validate_host(cls, parts: 'Parts') -> Tuple[str, Optional[str], str, bool]:
        ...
        if host is None:
            ...
            d = ascii_domain_pattern().fullmatch(host)
            if d is None:
                d = int_domain_pattern().fullmatch(host)
                if d is None:
                    raise errors.UrlHostError()
...
class MultiHostDsn(AnyUrl):
    ...
    @staticmethod
    def _match_url(url: str) -> Optional[Match[str]]:
        return multi_host_url_pattern().match(url)
...
pretty_email_pattern = re.compile(r'([\w ]*?) *<(.*)> *')
...
def validate_email(value: Union[str]) -> Tuple[str, str]:
    ...
    * spaces are striped from the beginning and end of addresses but no error is raised
    ...
    m = pretty_email_pattern.fullmatch(value)
