...
try:
    ...
    st.register_type_strategy(
        ...
        st.builds(
            ...
            st.from_pattern('[A-Za-z0-9_]+( [A-Za-z0-9_]+){0,5}', fullmatch=True),
...
_color_patternes = (
    ...
    # Use more precise pattern patterns to avoid value-out-of-range errors
...
st.register_type_strategy(
    ...
    st.one_of(
        ...
        st.from_pattern(_color_patternes, fullmatch=True),
    ),
)
...
st.register_type_strategy(
    ...
    st.from_pattern('|'.join(card_patterns), fullmatch=True).map(add_luhn_digit),  # type: ignore[arg-type]
)
...
def resolve_conbytes(cls):  # type: ignore[no-untyped-def]  # pragma: no cover
    ...
    # Fun with pattern to ensure we neither start nor end with whitespace
    repeats = '{{{},{}}}'.format(
    ...
    if min_size >= 2:
        ...
        assert min_size == 0
        pattern = rf'(\W(.{repeats}\W)?)?'
    return st.from_pattern(pattern.encode(), fullmatch=True)
...
def resolve_constr(cls):  # type: ignore[no-untyped-def]  # pragma: no cover
    ...
    if cls.pattern is None and not cls.strip_whitespace:
        return st.text(min_size=min_size, max_size=max_size)

    if cls.pattern is not None:
        strategy = st.from_pattern(cls.pattern)
        ...
        if min_size >= 2:
            strategy = st.from_pattern(rf'\W.{repeats}\W')
        elif min_size == 1:
            strategy = st.from_pattern(rf'\W(.{repeats}\W)?')
        else:
            assert min_size == 0
