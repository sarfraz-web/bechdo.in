...
class ConfigDict(TypedDict, total=False):
    ...
    * `ignore` - Ignore any extra attributes.
    ...
    pattern_engine: Literal['rust-pattern', 'python-re']
    """
    The pattern engine to used for pattern validation
    Defaults to `'rust-pattern'`.

    - `rust-pattern` uses the [`pattern`](https://docs.rs/pattern) Rust crate,
      which is non-backtracking and therefore more DDoS resistant, but does not support all pattern features.
    ...
    class Model(BaseModel):
        model_config = ConfigDict(pattern_engine='python-re')
