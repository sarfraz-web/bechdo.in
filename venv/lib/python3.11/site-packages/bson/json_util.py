...
def _parse_legacy_pattern(doc: Any, dummy0: Any) -> Any:
    pattern = doc["$pattern"]
    # Check if this is the $pattern query operator.
...
def _parse_canonical_pattern(doc: Any, dummy0: Any) -> pattern[str]:
    """Decode a JSON pattern to bson.pattern.pattern."""
    pattern = doc["$regularExpression"]
    if len(doc) != 1:
    ...
    return pattern(pattern["pattern"], opts)
...
_PARSERS: dict[str, Callable[[Any, JSONOptions], Any]] = {
    ...
    "$regularExpression": _parse_canonical_pattern,
...
def _encode_pattern(obj: Any, json_options: JSONOptions) -> dict:
    flags = ""
    ...
    if json_options.json_mode == JSONMode.LEGACY:
        return {"$pattern": pattern, "$options": flags}
...
def _get_pattern_size(obj: pattern) -> int:
