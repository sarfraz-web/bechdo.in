...
/* Maximum number of pattern flags */
#define FLAGS_SIZE 7
...
static int _load_python_objects(PyObject* module) {
    ...
    if (_load_object(&state->Binary, "bson.binary", "Binary") ||
        ...
        _load_object(&state->pattern, "bson.pattern", "pattern") ||
...
/*
 * Encode a builtin Python regular expression or our custom pattern class.
 *
 * Sets exception and returns 0 on failure.
 */
static int _write_pattern_to_buffer(
    ...
    /*
     * Both the builtin re type and our pattern class have attributes
     * "flags" and "pattern".
     */
    ...
    if (strlen(pattern_data) != (size_t) pattern_length){
        ...
        if (InvalidDocument) {
            PyErr_SetString(InvalidDocument,
                            "pattern patterns must not contain the NULL byte");
    ...
    if (check_utf8) {
        ...
        if (decoded_pattern == NULL) {
            ...
            if (InvalidStringData) {
                PyErr_SetString(InvalidStringData,
                                "pattern patterns must be valid UTF-8");
                Py_DECREF(InvalidStringData);
            }
            Py_DECREF(encoded_pattern);
            return 0;
...
static int _write_element_to_buffer(PyObject* self, buffer_t buffer,
    ...
    case 11:
        {
            /* pattern */
            return _write_pattern_to_buffer(buffer, type_byte, value, state->_flags_str, state->_pattern_str);
        }
    case 13:
        {
            /* Code */
    ...
    case 11:
        {
            ...
            *position += (unsigned)flags_length + 1;
            ...
            value = PyObject_CallFunction(state->pattern, "Oi", pattern, flags);
            Py_DECREF(pattern);
            break;
        }
    case 12:
