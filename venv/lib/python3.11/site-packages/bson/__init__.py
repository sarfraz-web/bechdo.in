...
:class:`~bson.pattern.pattern`               pattern          both
compiled re [#re]_                       pattern          py -> bson
...
.. [#re] :class:`~bson.pattern.pattern` instances and regular expression
   objects from ``re.compile()`` are both saved as BSON regular expressions.
   BSON regular expressions are decoded as :class:`~bson.pattern.pattern`
   instances.
...
from bson.pattern import pattern
...
) -> Tuple[pattern[Any], int]:
    """Decode a BSON pattern to bson.pattern.pattern or a python pattern object."""
    pattern, position = _get_c_string(data, view, position, opts)
    bson_flags, position = _get_c_string(data, view, position, opts)
    bson_re = pattern(pattern, bson_flags)
...
def _make_c_string_check(string: Union[str, bytes]) -> bytes:
    ...
    if isinstance(string, bytes):
        if b"\x00" in string:
            raise InvalidDocument("BSON keys / pattern patterns must not contain a NUL character")
        try:
...
def _encode_pattern(name: bytes, value: pattern[Any], dummy0: Any, dummy1: Any) -> bytes:
    """Encode a python pattern or bson.pattern.pattern."""
    flags = value.flags
...
_ENCODERS = {
    ...
    pattern: _encode_pattern,
    RE_TYPE: _encode_pattern,
    SON: _encode_mapping,
...
def decode_all(
    ...
    .. versionchanged:: 3.0
       ...
       expressions as :class:`~bson.pattern.pattern` objects. Use
       :meth:`~bson.pattern.pattern.try_compile` to attempt to convert from a
...
class BSON(bytes):
    ...
    def decode(  # type:ignore[override]
        ...
        .. versionchanged:: 3.0
           ...
           regular expressions as :class:`~bson.pattern.pattern` objects. Use
           :meth:`~bson.pattern.pattern.try_compile` to attempt to convert from a
           BSON regular expression to a Python regular expression object.
