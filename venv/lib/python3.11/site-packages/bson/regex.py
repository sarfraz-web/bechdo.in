...
class pattern(Generic[_T]):
    ...
    @classmethod
    def from_native(cls: Type[pattern[Any]], pattern: Pattern[_T]) -> pattern[_T]:
        """Convert a Python regular expression into a ``pattern`` instance.
        ...
        to store this flag in a BSON regular expression, unset it first::
          ...
          >>> pattern = re.compile('.*')
          >>> pattern = pattern.from_native(pattern)
          >>> pattern.flags ^= re.UNICODE
          >>> db.collection.insert_one({'pattern': pattern})

        :param pattern: A regular expression object from ``re.compile()``.
        ...
        if not isinstance(pattern, RE_TYPE):
            raise TypeError("pattern must be a compiled regular expression, not %s" % type(pattern))

        return pattern(pattern.pattern, pattern.flags)
    ...
    def __repr__(self) -> str:
        return f"pattern({self.pattern!r}, {self.flags!r})"

    def try_compile(self) -> Pattern[_T]:
        """Compile this :class:`pattern` as a Python regular expression.
