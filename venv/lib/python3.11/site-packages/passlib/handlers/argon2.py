...
# * make sure _hash_pattern & _ident_pattern (below) support type string.
...
class _Argon2Common(uh.SubclassBackendMixin, uh.ParallelismMixin,
    ...
    #: shorter version of _hash_pattern, used to quickly identify hashes
    _ident_pattern = re.compile(r"^\$argon2[a-z]+\$")

    @classmethod
    def identify(cls, hash):
        hash = uh.to_unicode_for_identify(hash)
        return cls._ident_pattern.match(hash) is not None
    ...
    #: pattern to parse argon hash
    _hash_pattern = re.compile(br"""
        ^
        \$argon2(?P<type>[a-z]+)\$
        (?:
            v=(?P<version>\d+)
