...
PARAM_pattern = re.compile("{([a-zA-Z_][a-zA-Z0-9_]*)(:[a-zA-Z_][a-zA-Z0-9_]*)?}")
...
def compile_path(
    ...
    of (pattern, format, {param_name:convertor}).
    ...
    for match in PARAM_pattern.finditer(path):
        ...
        path_pattern += f"(?P<{param_name}>{convertor.pattern})"
    ...
    if is_host:
        ...
        path_pattern += re.escape(hostname) + "$"
    else:
        path_pattern += re.escape(path[idx:]) + "$"

    path_format += path[idx:]

    return re.compile(path_pattern), path_format, param_convertors
...
class Route(BaseRoute):
    def __init__(
        ...
        *,
        ...
        self.path_pattern, self.path_format, self.param_convertors = compile_path(path)

    def matches(self, scope: Scope) -> typing.Tuple[Match, Scope]:
        if scope["type"] == "http":
            match = self.path_pattern.match(scope["path"])
            if match:
...
class Mount(BaseRoute):
    def __init__(
        ...
        *,
        ...
        self.name = name
        self.path_pattern, self.path_format, self.param_convertors = compile_path(
            self.path + "/{path:path}"
        )
...
class Host(BaseRoute):
    def __init__(
        ...
        self.app = app
        self.name = name
        self.host_pattern, self.host_format, self.param_convertors = compile_path(host)
    ...
    def matches(self, scope: Scope) -> typing.Tuple[Match, Scope]:
        if scope["type"] in ("http", "websocket"):
            ...
            match = self.host_pattern.match(host)
            if match:
                matched_params = match.groupdict()
                for key, value in matched_params.items():
                    matched_params[key] = self.param_convertors[key].convert(value)
