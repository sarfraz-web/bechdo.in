...
class ServerProtocol(Protocol):
    ...
    def process_origin(self, headers: Headers) -> Origin | None:
        ...
        if self.origins is not None:
            for origin_or_pattern in self.origins:
                if origin_or_pattern == origin or (
                    isinstance(origin_or_pattern, re.Pattern)
                    and origin is not None
                    and origin_or_pattern.fullmatch(origin) is not None
                ):
                    break
            else:
