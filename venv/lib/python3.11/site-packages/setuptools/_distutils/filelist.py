...
class FileList:
    ...
    def include_pattern(
        ...
        If 'is_pattern' is true, 'anchor' and 'prefix' are ignored, and
        'pattern' is assumed to be either a string containing a pattern or a
        pattern object -- no translation is done, the pattern is just compiled
        and used as-is.
        ...
        files_found = False
        pattern_re = translate_pattern(pattern, anchor, prefix, is_pattern)
        self.debug_print(f"include_pattern: applying pattern r'{pattern_re.pattern}'")
    ...
    def exclude_pattern(
        ...
        files_found = False
        pattern_re = translate_pattern(pattern, anchor, prefix, is_pattern)
        self.debug_print(f"exclude_pattern: applying pattern r'{pattern_re.pattern}'")
...
def glob_to_re(pattern):
    ...
    a string containing the pattern.  Differs from 'fnmatch.translate()' in
    ...
    sep = os.sep
    if os.sep == '\\':
        # we're using a pattern to manipulate a pattern, so we need
...
def translate_pattern(pattern, anchor=True, prefix=None, is_pattern=False):
    """Translate a shell-like wildcard pattern to a compiled regular
    expression.  Return the compiled pattern.  If 'is_pattern' true,
    then 'pattern' is directly compiled to a pattern (if it's a string)
    or just returned as-is (assumes it's a pattern object).
    """
    if is_pattern:
