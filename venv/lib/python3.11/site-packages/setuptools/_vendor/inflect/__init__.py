...
def joinstem(cutpoint: Optional[int] = 0, words: Optional[Iterable[str]] = None) -> str:
    """
    Join stem of each word in words into a string for pattern.
...
class engine:
    ...
    def checkpat(self, pattern: Optional[Word]) -> None:
        """
        check for errors in a pattern pattern
        """
        if pattern is None:
            return
        try:
    ...
    def checkpatplural(self, pattern: Optional[Word]) -> None:
        """
        check for errors in a pattern replace pattern
        """
        return
    ...
    def _indef_article(self, word: str, count: Union[int, str, Any]) -> str:
        ...
        matches = (
            f'{article} {word}'
            for patternen, article in self._indef_article_cases
            if patternen.search(word)
        )
    ...
    def present_participle(self, word: Word) -> str:
        ...
        for patternen, repl in PRESENT_PARTICIPLE_REPLACEMENTS:
            ans, num = patternen.subn(repl, plv)
            if num:
                return f"{ans}ing"
        return f"{ans}ing"
