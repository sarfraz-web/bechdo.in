...
# IP patternes retained for backwards compatibility
...
class S3ArnParamHandler:
    ...
    _OUTPOST_RESOURCE_pattern = re.compile(
    ...
    def _add_resource_type_and_name(self, arn, arn_details):
        match = self._RESOURCE_pattern.match(arn_details['resource'])
        if match:
    ...
    def _store_outpost(self, params, context, arn_details):
        ...
        match = self._OUTPOST_RESOURCE_pattern.match(resource_name)
        if not match:
...
class S3ControlEndpointSetter:
    _DEFAULT_PARTITION = 'aws'
    _DEFAULT_DNS_SUFFIX = 'amazonaws.com'
    _HOST_LABEL_pattern = re.compile(r'^[a-zA-Z0-9\-]{1,63}$')
    ...
    def _is_valid_host_label(self, label):
        return self._HOST_LABEL_pattern.match(label)
...
class S3ControlArnParamHandler:
    ...
    _RESOURCE_SPLIT_pattern = re.compile(r'[/:]')
    ...
    def _split_resource(self, arn_details):
        return self._RESOURCE_SPLIT_pattern.split(arn_details['resource'])
